...
class FilesAdapter(private val rootDir: String) : FileBundleAdapter<List<File>> {
    ...
    private fun makeNestedDirsIfNecessary(it: dreifa.app.fileBundle.BundleItem) {
        if (it.path.contains("/")) {
            val nested = it.path
                .replaceAfterLast("/", "", "")
                .removeSuffix("/")
    ...
    override fun toBundle(adapted: List<File>): FileBundle {
        ...
        val bundleItems = ArrayList<dreifa.app.fileBundle.BundleItem>()
        ...
        (1..fileCount).forEach {
            ...
            when (type) {
                "TextBundleItem" -> {
                    bundleItems.add(dreifa.app.fileBundle.TextBundleItem(path, file.readText(), isExecutable))
                }
                "BinaryBundleItem" -> {
                    bundleItems.add(dreifa.app.fileBundle.BinaryBundleItem(path, file.readBytes(), isExecutable))
                }
            }
        }
